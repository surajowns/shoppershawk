<?php
  $product=App\Product::with('productImage','wishlist','productRating')->where('type',3)->where('status',1)->where('qty','!=',0)->orderBy('qty','DESC')->get()->toArray();
  $cate=array();
                            ...
                            <div class="product_items {{$productdetails['qty'] <= 0?'not_in_stock':''}}">
                                    <figure class="single_product ">
                                        ...
                                        @if(!empty($productdetails['product_image']))
                                           <a class="primary_img" href="{{url('/product_details/'.$productdetails['slug'])}}" target="_blank" title="{{$productdetails['name']}}"><img class="best-selling" src="{{url('public/product_image/'.$productdetails['product_image'][0]['image'])}}" alt="{{$productdetails['name']}}" /></a>
                                      @endif 
  
                                        </div>
                                        <div class="product_content">
                                            <h4 class="product_name"><a href="{{url('/product_details/'.$productdetails['slug'])}}" target="_blank" title="{{$productdetails['name']}}">{{ucfirst($productdetails['name'])}}</a></h4>
