<?php
$user=Auth::user();
  $newarrivalproduct=App\Product::with(['productImage','wishlist'=>function($query) use ($user){$query->select('*')->where('user_id',isset($user)?$user->id:'');}])->where('type',4)->where('status',1)->where('qty','!=',0)->orderBy('qty','DESC')->get()->toArray();
